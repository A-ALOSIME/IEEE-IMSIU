...
try{
 ...
 await page.goto(origin+'/events/archive',{waitUntil:'networkidle'});assert.equal(await page.locator('.archive-card').count(),26);assert.ok(await page.locator('.archive-card img').first().evaluate(image=>image.naturalWidth>0));assert.match(await page.locator('.archive-card').first().innerText(),/18 أكتوبر 2020/);await page.locator('.archive-card').first().click();assert.equal(await page.locator('.event-facts dd').first().textContent(),'د. عبدالعزيز الباتلي');
 await page.goto(origin+'/collaboration',{waitUntil:'networkidle'});assert.equal(await page.locator('.collaboration-contact img').count(),0);const slider=page.locator('.partner-logos');await slider.scrollIntoViewIfNeeded();const sliderBox=await slider.boundingBox(),before=await slider.evaluate(element=>element.scrollLeft);await page.mouse.move(sliderBox.x+sliderBox.width/2,sliderBox.y+sliderBox.height/2);await page.mouse.down();await page.mouse.move(sliderBox.x+100,sliderBox.y+sliderBox.height/2,{steps:8});await page.mouse.up();const after=await slider.evaluate(element=>element.scrollLeft);assert.notEqual(after,before);
